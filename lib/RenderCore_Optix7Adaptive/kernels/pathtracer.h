...
//  +-----------------------------------------------------------------------------+
__host__ void shade( const int pathCount, float4* accumulator, float4* features, const uint stride,
	float4* pathStates, float4* hits, float4* connections,
	const uint R0, const uint shift, const uint* blueNoise, const int pass,
	...
	const float3 p1, const float3 p2, const float3 p3, const float3 pos )
{
	...
	shadeKernel << <gridDim.x, 128 >> > (accumulator, features, stride, pathStates, hits, connections, R0, shift, blueNoise,
