...
__device__ void setupPrimaryRay( const uint pathIdx, const uint stride )
{
	...
	if (pathIdx < stride /* OptiX bug workaround? */) if (u2 != 0xffffffff) /* bandwidth reduction */
...
__device__ void generateShadowRay( const uint rayIdx, const uint stride )
{
	...
	if (pixelIdx < stride /* OptiX bug workaround? */) params.frameAcc[pixelIdx] += make_float4( E4.x, E4.y, E4.z, 0 );
}
