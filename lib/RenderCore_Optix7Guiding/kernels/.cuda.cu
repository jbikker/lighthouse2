...
__constant__ uint4* guidance = 0;		// PNEE guidance data
__constant__ float3 PNEEmin, PNEErext;	// origin of PNEE grid, reciprocal of its extent
...
{
	if (id == PMIN) cudaMemcpyToSymbol( PNEEmin, &v, sizeof( float3 ) );
	if (id == PEXT) cudaMemcpyToSymbol( PNEErext, &v, sizeof( float3 ) );
}
...
{
	stagePtrCpy( GUIDANCE, g );
	stageFloat3Cpy( PMIN, bmin );
	stageFloat3Cpy( PEXT, rext );
}
