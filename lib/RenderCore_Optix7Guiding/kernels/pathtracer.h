	...
	const uint pathCount )
{
	...
	if ((FLAGS & S_SPECULAR) == 0 && connections != 0) // skip for specular vertices
	{
		...
		if (pixelIdx % SCRWIDTH < SCRWIDTH / 2)
		{
			...
			L = RandomPointOnLightPNEE( r4.x, r4.y, I, II, fN * faceDir, pickProb, lightPdf, lightColor ) - I;
		}
		else
		{
			L = RandomPointOnLight( r4.x, r4.y, I, fN * faceDir, pickProb, lightPdf, lightColor ) - I;
		}
