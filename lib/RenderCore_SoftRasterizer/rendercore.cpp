...
void RenderCore::Render( const ViewPyramid& view, const Convergence converge, const float brightness, const float contrast )
{
	...
	const float3 Z = normalize( view.pos - (view.p2 + view.p3) * 0.5f );
	transform[0] = X.x, transform[4] = X.y, transform[8] = X.z, transform[3] = view.pos.x;
	transform[1] = Y.x, transform[5] = Y.y, transform[9] = Y.z, transform[7] = view.pos.y;
	transform[2] = Z.x, transform[6] = Z.y, transform[10] = Z.z, transform[11] = view.pos.z;
	rasterizer.Render( transform );
